    ...
    try:
        ...
        if hasattr(result, 'obb'):
            ...
            if coin_detections:
                ...
                # Assuming the width and height are at indices 2 and 3
                if len(coin) >= 4:
                    width_px = coin[2]
                    height_px = coin[3]
                ...
                if px_to_mm_ratio is not None:
                    ...
                    for det in obb_detections:
                        if len(det) > 5:
                            class_id = int(det[5])
                            # Assuming bounding box coordinates are at indices 0, 1, 2, 3
                            if class_id != COIN_CLASS_ID and len(det) >= 4:
                                # For OBB, these might not directly represent min/max x, y
                                # You might need to calculate the oriented bounding box dimensions differently
                                # based on the 'xywhr' attribute if that's more accurate for length
                                # For a quick estimate using the axis-aligned bounding box from OBB:
                                x_center, y_center, width_obb, height_obb, angle, conf, cls = det
                                length_px = max(width_obb, height_obb) # Approximate length
                                length_mm = length_px * px_to_mm_ratio
                                screw_lengths.append((int(cls), length_mm))
